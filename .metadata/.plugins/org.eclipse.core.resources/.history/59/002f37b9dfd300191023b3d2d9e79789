...
public class OccurrenceDetector {
	//private static String[] names = {"Occ_SugarDetected", "Occ_CakeAvaliable"};
	
	public static boolean detectOcurrance(String keyword){
		switch(keyword){
			case "Occ_SugarDetected":
	...
	public static boolean checkNeedForOcurranceDetector(String name){
		String[] names = {"Occ_SugarDetected", "Occ_CakeAvaliable"};
...
//	}
	...
	public static boolean detectHighSugar(){
		...
		MySQL_Connection conn1 = new MySQL_Connection();
		conn1.deleteGloceries();
		try{
			//URIBuilder builder = new URIBuilder("https://dev.tescolabs.com/grocery/products/?query={query}&offset={offset}&limit={limit}");
			URIBuilder builder = new URIBuilder("https://dev.tescolabs.com/grocery/products/?query=cake&offset=0&limit=100");
...
//						}
						...
						//if(td.Product_Name.indexOf("sugar") != 1 || td.Product_Name.indexOf("Sugar")  != 1 || td.Super_Department.indexOf("sugar")  != -1 || td.Super_Department.indexOf("Sugar")  != 1 || td.Department.indexOf("sugar")  != -1 || td.Department.indexOf("Sugar")  != 1 || td.Description.indexOf("sugar")  != 1 || td.Description.indexOf("Sugar")  != 1){
							//if(td.Product_Name.indexOf("chocolate") != -1 || td.Product_Name.indexOf("Chocolate")  != -1 || td.Super_Department.indexOf("chocolate")  != -1 || td.Super_Department.indexOf("Chocolate")  != -1 || td.Department.indexOf("chocolate")  != -1 || td.Department.indexOf("Chocolate")  != -1 || td.Description.indexOf("chocolate")  != -1 || td.Description.indexOf("Chocolate")  != -1){	
...
//									        	    	    System.out.println("SugarDetected");
//									        	    	    // Put things back
//									        	    	    System.out.flush();
//									        	    	    System.setOut(old1);
//									        	    	    // Show what happened
//									        	    	    System.out.println(sugarCheck.toString() + "On this Cake" );
							System.out.println("Sugar detected on these/this Cake(s)" );

							conn.insertGloceris(td,"Sugar");
							count ++;
							}else{
								conn.insertGloceris(td,"No sugar");
							}
						}catch(JSONException e){
								e.printStackTrace();
						}
...
//	    	}
	    	...
	    	if(detectHighSugar()){
	    		...
	    		System.out.println("NO Sugar DETECTED");
	    	}
	    }
